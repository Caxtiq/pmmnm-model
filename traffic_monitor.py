...
class TrafficDensityMonitor:
    ...
    def process_frame(
        ...
        vehicle_counts = self.count_vehicles(
            results,
            location_config['count_vehicle_types']
        )
    ...
    def save_detection_image(
        ...
        filepath = os.path.join(
            self.config['logging']['detections_dir'],
            filename
        )
                    ...
                    if total_count >= density_threshold:
                        ...
                        try:
                            ...
                            if response.get('thresholdExceeded'):
                                self.logger.warning("🚨 Sensor threshold exceeded on server!")
                        ...
                        self.logger.info(f"✓ Normal density (threshold: {density_threshold})")
    ...
    def run_all_locations(self):
        ...
        self.logger.info(f"Starting monitoring for all {len(self.config['locations'])} locations")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Traffic Density Monitor using YOLO"
    )
    parser.add_argument(
        '--config',
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help='Location index to monitor (default: 0)'
    )
    parser.add_argument(
        '--all',
        action='store_true',
        help='Monitor all locations sequentially'
    )
